...
مقدار `version` باید `0.4.1` باشد و `woocommerce`، `product_cat` و `yoast` بهتر است `true` باشند.
...
- فایل نسخه `0.4.1` را جایگزین نسخه قبلی و کش سایت را پاک کنید.
...
1. پاسخ `/health` باید نسخه `0.4.1` را نشان دهد.
